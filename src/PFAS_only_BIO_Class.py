...
# df_bio = df.copy()
# df_bio = df_bio.drop(columns=list_bio)     # drop the pfas bio cols
df_bio = df.copy()[basic_features]

# label data PFAS_total_BIO-> BIO_label
df_bio.loc[(df_bio.PFAS_total_BIO <= 0), 'BIO_label'] = 0
df_bio.loc[(df_bio.PFAS_total_BIO > 0 ),'BIO_label'] = 1
# df_bio.loc[((df_bio.PFAS_total_BIO > 0 ) & (df_bio.PFAS_total_BIO <= 110000 )),'BIO_label'] = 1
# df_bio.loc[(df_bio.PFAS_total_BIO > 110000), 'BIO_label'] = 2
df_bio = df_bio.drop(columns=['PFAS_total_BIO'])  # 'PFAS_total_EFF',
print(df_bio['BIO_label'].value_counts())    # get the category & their numbers in the label col

df_bio = df_bio.rename(columns = lambda x:re.sub('[^A-Za-z0-9_]+', '', x))

# split into X and y matrices
num_col = df_bio.shape[1]   # 1 - return number of cols
print('num_col', num_col)
X = df_bio.iloc[:, :-1]     # delete .values for rf importance
y = df_bio.iloc[:, -1]
...
for classifier_name, classifier_info in classifiers.items():
    ...
    with open(f'{classifier_name}_model_BIO_web.pkl', 'wb') as f:
        pickle.dump(clf, f)
